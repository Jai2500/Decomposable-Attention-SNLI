...
parser.add_argument(
    "--train_file",
    type=str,
    default="data/entail-train.hdf5",
    help="training data file (hdf5)",
)
parser.add_argument(
    "--val_file",
    type=str,
    default="data/entail-val.hdf5",
    help="validation data file (hdf5)",
)
parser.add_argument(
    "--test_file",
    type=str,
    default="data/entail-test.hdf5",
    help="test data file (hdf5)",
)
parser.add_argument(
    "--max_length",
    type=int,
    default=82,
    help="maximum length of training sentences. -1 means no max length",
)
parser.add_argument(
    "--w2v_file",
    type=str,
    default="data/w2v.hdf5",
    help="pretrained word vectors file (hdf5)",
)

parser.add_argument(
    "--embedding_size", type=int, default=300, help="word embedding size"
)
...
parser.add_argument(
    "--gpus", type=int, default=0, help="number of gpus to train on. -1 for all gpus"
)
parser.add_argument(
    "--val_interval",
    type=int,
    default=500,
    help="interval for checking the validation dataset",
)
parser.add_argument(
...
parser.add_argument(
    "--optimizer", type=str, default="adagrad", choices=["adam", "adagrad"]
)
...
parser.add_argument(
    ...
    type=float,
    default=5,
    help="if the norm of the gradient vector exceeds this renormalize it to have the norm equal to the max_grad_norm",
)
parser.add_argument(
    "--param_init", type=float, default=0.01, help="parameter initialization gaussian"
)
parser.add_argument(
    "--weight_decay", type=float, default=5e-5, help="l2 regularization"
)
...
atten = Atten(
    hidden_size=args.hidden_size, label_size=TRAIN_LBL_SIZE, param_init=args.param_init
)
