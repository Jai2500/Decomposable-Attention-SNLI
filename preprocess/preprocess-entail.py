...
class Indexer:
    ...
    def convert(self, w):
        return (
            self.d[w]
            if w in self.d
            else self.d["<oov" + str(np.random.randint(1, 100)) + ">"]
        )
...
def get_data(args):
    ...
    for i in range(
        1, 101
    ):  # hash oov words to one of 100 random embeddings, per Parikh et al. 2016
        oov_word = "<oov" + str(i) + ">"
        word_indexer.vocab[oov_word] += 1

    def make_vocab(srcfile, targetfile, labelfile, seqlength):
        num_sents = 0
        for _, (src_orig, targ_orig, label_orig) in enumerate(
            zip(open(srcfile, "r"), open(targetfile, "r"), open(labelfile, "r"))
        ):
    ...
    def convert(
        ...
        both_lengths = np.zeros(
            num_sents, dtype={"names": ["x", "y"], "formats": ["i4", "i4"]}
        )
        dropped = 0
        sent_id = 0
        for _, (src_orig, targ_orig, label_orig) in enumerate(
    ...
    num_sents_train = make_vocab(
        args.srcfile, args.targetfile, args.labelfile, args.seqlength
    )
    print("Number of sentences in training: {}".format(num_sents_train))
    num_sents_valid = make_vocab(
        args.srcvalfile, args.targetvalfile, args.labelvalfile, args.seqlength
    )
    print("Number of sentences in valid: {}".format(num_sents_valid))
    num_sents_test = make_vocab(
        args.srctestfile, args.targettestfile, args.labeltestfile, args.seqlength
    )
    ...
    print(
        "Source vocab size: Original = {}, Pruned = {}".format(
            len(word_indexer.vocab), len(word_indexer.d)
        )
    )
    print(
...
def main(arguments):
    ...
    parser.add_argument(
        "--srcfile",
        help="Path to sent1 training data.",
        default="data/entail/src-train.txt",
    )
    parser.add_argument(
        "--targetfile",
        help="Path to sent2 training data.",
    ...
    parser.add_argument(
        "--srcvalfile",
        help="Path to sent1 validation data.",
        default="data/entail/src-dev.txt",
    )
    parser.add_argument(
        "--targetvalfile",
        help="Path to sent2 validation data.",
        default="data/entail/targ-dev.txt",
    )
    parser.add_argument(
        "--labelvalfile",
        help="Path to label validation data.",
        default="data/entail/label-dev.txt",
    )
    parser.add_argument(
        "--srctestfile",
        help="Path to sent1 test data.",
        default="data/entail/src-test.txt",
    )
    parser.add_argument(
        "--targettestfile",
        help="Path to sent2 test data.",
